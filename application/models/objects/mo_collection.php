...
class MO_Collection {
	...
	public function clear(){
		$this->library=Array();
		if(count($this->library)==0)return true;
		else return false;
	}
	...
	public function getByKey($id){
		if(isset($this->library[$id])) return $this->library[$id];
		else return false;
	}
	...
	public function getNewCollectionByVal($var,$list){
		...
		foreach ($this->library as $offer) {
			if(in_array($offer->{$var},$list)) $lib->add($offer);
		}
		return $lib;
	}
	...
	public function getVar($var,$unique = false){
		...
		if($unique==true)$list=array_unique($list);
		return $list;
	}
	...
	public function getItemByVar($key,$var){
		foreach($this->library as $item){
			if(isset($item->{$key}) && $item->{$key}==$var) return $item;
		}
		return false;
	}
    ...
    public function getItemBySubString($key,$var){
        foreach($this->library as $item){
            if(isset($item->{$key})){
                if(strpos($item->{$key},$var)!==false) return $item;
            }
        }
        return false;
    }
